...
*/
static ZydisStatus ParseOperandMem(const ZydisFormatter *formatter,
                                   ZydisString *string,
                                   const ZydisDecodedInstruction *instruction,
    ...
    return ptrParseOperandMem(formatter, string, instruction, operand,
                              userData);
}

/*unsigned char hook_head[] = {
        0x48,0xbb,0x35,0x08,0x40,0x00,0x00,0x00,0x00,0x00,//   mov rbx,
0x0000000000400835 0xff, 0xe3 //jmp rbx
};*/
...
static MemChunk *TryAllocImpl(void *tryaddr, void *targetaddr) {
    MemChunk *res =
        (MemChunk *)mmap(tryaddr, MCHUNK_SZ, PROT_READ | PROT_WRITE | PROT_EXEC,
                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (ADDR_OK(res, targetaddr)) {
        res->init();
        return res;
    }
...
static MemChunk *TryCreateChunk(void *address) {
    ...
    if (addr == 0) {
        MemChunk *res = (MemChunk *)mmap(
            addr, MCHUNK_SZ, PROT_READ | PROT_WRITE | PROT_EXEC, flags, -1, 0);
        res->init();
        return res;
    }
    if (auto res = TryAllocImpl(address, address)) {
        return res;
    }
    const uintptr_t search_step = (MCHUNK_SZ << 1);
    uintptr_t lowerbound = addr;
    uintptr_t upperbound = addr;
    uintptr_t diff = 0;
    for (;;) {
        if (lowerbound >= search_step) {
            lowerbound -= search_step;
            if (auto res = TryAllocImpl((void *)lowerbound, address)) {
                return res;
            }
        }
        if (upperbound < std::numeric_limits<uintptr_t>::max() - search_step) {
            upperbound += search_step;
            if (auto res = TryAllocImpl((void *)upperbound, address)) {
                return res;
            }
        }
        diff += search_step;
...
inline bool InsertPatchPoint(PatchInfo *pool, int &num_patch_points,
                             PatchType type, int offset) {
    ...
    pinfo->patch_addr_offset =
        offset; //(readPointer + len) - (uint8_t*)oldfunc;
    return false;
}

#define FHASSERT(cond, pinst)                                                  \
    do {                                                                       \
        if (!(cond)) {                                                         \
            fprintf(stderr,                                                    \
                    "PFishHook is unable to patch this instructions with "     \
                    "RIP: %lx\n",                                              \
                    *(uint64_t *)pinst);                                       \
...
static inline HookStatus RangeProtect(void *start, size_t size, int wrable) {
    ...
    if (mprotect(start2, (char *)end - (char *)start2,
                 PROT_READ | PROT_EXEC | (wrable ? PROT_WRITE : 0)) < 0)
        return FHMprotectFail;
    return FHSuccess;
}

/*
Do Hook. It will replace the head of the function "oldfunc" with a "jmp" to the
function "newfunc", and copy the the head of the function "oldfunc" to newly
alloacted space ("jump space"), returning the pointer to the "jump space" with
"poutold". If success, return "FHSuccess"
*/
HookStatus HookItSafe(void *oldfunc, void **poutold, void *newfunc,
                      int need_checking, void *suggested_address) {
    ...
    if (PageSize2 == 0) {
        ...
        ZydisFormatterSetHook(&formatter,
    ...
    while (ZYDIS_SUCCESS(ZydisDecoderDecodeBuffer(
        ...
        if (instruction.attributes & ZYDIS_ATTRIB_IS_RELATIVE) {
            ...
            for (int i = 0; i < instruction.operandCount;
                 i++) // for all operands, check if it is RIP-relative
            {
                auto operand = &instruction.operands[i];
                if (operand->type == ZYDIS_OPERAND_TYPE_MEMORY &&
                    operand->mem.disp.hasDisplacement) {
                    FHASSERT(operand->mem.base != ZYDIS_REGISTER_EIP,
                             instruction.instrAddress);
                    if (operand->mem.base == ZYDIS_REGISTER_RIP) {
                        FHASSERT(instruction.raw.disp.size == 32,
                                 instruction.instrAddress);
                        patched = true;
                        /*
                        NOTE: We might meet jmp QWORD PTR [rip].While this instr
                        reads mem at rip+6 (we will modify this address), we
                        should copy QWORD rip+6 to shadow function
                         */
                        auto pOffset = (int32_t *)(readPointer +
                                                   instruction.raw.disp.offset);
                        auto word_offset = (readPointer + *pOffset) -
                                           (uint8_t *)oldfunc +
                                           instruction.length;
                        // printf("off %d\n",*pOffset);
                        // jmpq QWORD ptr [rip+0] (6) QWORD (8)
                        if (word_offset < jmplen &&
                            instruction.mnemonic >= ZYDIS_MNEMONIC_JB &&
                            instruction.mnemonic <= ZYDIS_MNEMONIC_JZ) {
                            // Uhh We should copy this word to shadow function
                            // In most cases,This only happen when a function is
                            // hooked
                            FHASSERT(QWORDPointer == nullptr,
                                     instruction.instrAddress);
                            QWORDPointer =
                                readPointer + (*pOffset) + instruction.length;
                        } else {
                            if (InsertPatchPoint(PatchInfoPool,
                                                 num_patch_points,
                                                 FHPatchLoad32,
                                                 (readPointer +
                                                  instruction.raw.disp.offset) -
                    ...
                    if (operand->imm.isSigned && operand->imm.isRelative) {
                        if (instruction.raw.imm[0].size == 8) {
                            patched = true;
                            if (InsertPatchPoint(
                                    PatchInfoPool, num_patch_points,
                                    FHPatchJump8,
                                    (readPointer +
                                     instruction.raw.imm[0].offset) -
                                        (uint8_t *)oldfunc))
                                return FHTooManyPatches;
                            // if there is a jmp instruction, we need one more
                            // "jmp" in our jump space to patch it, so add
                            // alloc_size with GetJmpLen()
    ...
    */
    size_t alloc_size =
        length + GetJmpLenLarge() + sizeof(size_t) + patch_jump_bed_size;
    ...
    *(size_t *)outfunc = length;
    ...
    for (int i = 0; i < num_patch_points; i++) {
        ...
        case FHPatchLoad32:
            ...
            int32_t offset;
            offset = *(int32_t *)((unsigned char *)oldfunc +
                                  PatchInfoPool[i].patch_addr_offset);
            // calculate the new relative offset
            int64_t delta;
            delta = (int64_t)offset - ((char *)outfunc - (char *)oldfunc);
            if (delta > INT_MAX || delta < INT_MIN) {
                ...
                return HookItSafe(oldfunc, poutold, newfunc, need_checking,
                                  oldfunc);
            }
            // the patch point in copied function
            int32_t *patch_point;
            patch_point =
                (int32_t *)(outfunc + PatchInfoPool[i].patch_addr_offset);
            *patch_point = delta;
            break;
        case FHPatchJump8:
            // if there is a jne "near" relative instr, the offset is too small
            // to jump from jump space to oldfun's body. So we first "jne near"
            // to a place in jump space, and then use our "far jmp" to jump to
            // the target
            unsigned char *patch_addr_jne; // the address of the instruction in
                                           // old function
            patch_addr_jne = (unsigned char *)oldfunc +
                             PatchInfoPool[i].patch_addr_offset - 1;
            // the address of the instruction in new function
            char *patch_instruction;
            patch_instruction =
                (char *)(outfunc + PatchInfoPool[i].patch_addr_offset - 1);
            uintptr_t target;
            target = (uintptr_t)patch_addr_jne + 2 + patch_addr_jne[1];

            // check if the jump target is within the copied part of the
            // function. If so, no need to patch
            int jump_target_offset;
            jump_target_offset = (char *)target - (char *)oldfunc;
            ...
            [a "jmp" instruction to original function] ----  length=
            "GetJmpLen()" [several "jmp" instructions to handle near jmp(s)]
            */
            int delta8;
            delta8 = outfunc + length + GetJmpLenLarge() +
                     jump_bed_num * GetJmpLenLarge() - (patch_instruction + 2);
            if (delta8 > 127 || delta8 < -128) {
                return FHPatchFailed;
            }
            patch_instruction[1] = delta8;

            char *jmp_bed = outfunc + length + GetJmpLenLarge() +
                            jump_bed_num * GetJmpLenLarge();
    ...
    *poutold = (void *)outfunc;
    ...
    if (RangeProtect(oldfunc, length, 1) != FHSuccess)
        return FHMprotectFail;
    // replace the head of oldfunc with newfunc
    GenerateJmp((char *)oldfunc, newfunc);
    // fill the gap with "int 3"
    memset((char *)oldfunc + GetJmpLen((char *)oldfunc, newfunc), 0xcc,
           length - GetJmpLen((char *)oldfunc, newfunc));
    // restore the protection
    if (RangeProtect(oldfunc, length, 0) != FHSuccess)
        return FHMprotectFail;
    return FHSuccess;
}
